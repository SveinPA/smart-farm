...
```
┌─────────────────┐         ┌─────────────┐         ┌─────────────────┐
│  Sensor Nodes   │ ◄─────► │   Broker    │ ◄─────► │ Control Panels  │
│                 │   TCP   │             │   TCP   │                 │
│ - Sensors       │         │  Message    │         │  - Visualization│
│ - Actuators     │         │  Router     │         │  - Commands     │
└─────────────────┘         └─────────────┘         └─────────────────┘
```
...
- Flexible actuator commands — broadcast to ALL sensor nodes **(EXTRA #5)**
...
**Easiest way to launch the entire system with one command:**
...
1. Builds the project (`mvn clean install`)
2. Starts the broker in the background
3. Starts a sensor node (all sensors) in the background
4. Launches the control panel GUI

**Logs:** Check `logs/` folder for component output (`broker.log`, `sensor-node.log`)

**To stop all components:**
- **Windows (PowerShell):** `.\stop-all.bat`
- **Windows (CMD):** `stop-all.bat`
- **Unix/Mac/Linux:** `./stop-all.sh`

---

## Getting Started 🛠️

**For manual step-by-step launch (advanced users):**
...
│   └── protocol/        # Message DTOs and parsers
├── broker/              # Central message broker
│   └── infra/network/   # TCP server and client handling
├── sensor-node/         # Sensor and actuator simulation
│   ├── entity/          # Sensor and actuator implementations
│   └── logic/           # Business logic and communication
└── control-panel/       # GUI application
│    ├── ui/              # JavaFX views and controllers
│    └── infra/network/   # Network communication
│
├── protocol.md           # Documentation of all message types and flows
├── README.md             # This file
...
- **10004 (LordManBat)** – Actuators, documentation, GUI
- **10050 (SveinPA)** – Broker, protocol, registry, message handling
- **10012 (Andsandn)** – Control panel UI/GUI, panel logic, communication
- **10024 (captnsundy)** – Sensor node logic, NodeAgent, documentation
...
**Note on GitHub Copilot Agent:**  
During code review of a large pull request, GitHub Copilot Agent was unintentionally activated.
While using Copilot to review the PR for spelling errors and minor issues, we clicked options in the Copilot review interface that we believed would generate review comments for manual correction.
However, this accidentally triggered Copilot's Agent Mode, which automatically committed fixes directly to the branch. These automated fixes were limited to spelling corrections in comments and minor non-functional improvements. No application logic was generated by the agent.  
This is why GitHub Copilot appears in the repository's Contributors tab. All fixes were subsequently reviewed and verified by team members before merging.

## License ©️
This project was created as part of the **IDATA2304 Computer Communication & Network Programming** course at **NTNU Ålesund.** It is intended for educational use in the context of the course assignment.